...
const Products = ({ product, handleDelete }) => {
  ...
  if (loading) {
    ...
    return (
        <div >
            <div className="max-w-sm h-[550px] mb-6 relative bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="p-5">
                   ...
                   <Link to={`/inventory/${_id}`} className="inline-flex  items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 ">
                    ...
                    {
                        user ? <button onClick={() =>handleDelete(_id)} className="btn ml-14 btn-circle  btn-ghost bg-rose-400"><i className="fa-solid text-xl  fa-trash-can"></i></button> : ''
                    }
                   </div>
                </div>
            </div>
